...
-- [TODO: Escribe tu consulta aquí]
  SELECT nombre, email,ciudad,rol_id,activo
  FROM usuarios
  WHERE activo = 1 AND (ciudad = 'Garzón' OR ciudad = 'Neiva') AND email LIKE '%@misena.edu.co%';
...
--- 5A: Clientes sin pedidos
SELECT 
  usr.nombre AS nombre_cliente,
  usr.email,
  usr.ciudad
FROM usuarios usr
LEFT JOIN pedidos ped ON usr.id = ped.usuario_id
WHERE ped.id IS NULL AND usr.rol_id = 3; -- Solo clientes

-- 5B: Categorías sin productos
SELECT 
