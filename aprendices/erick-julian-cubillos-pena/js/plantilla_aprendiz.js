 ...
 * 
 */
// Objeto simulando petición HTTP o registro de BD:
...
.filter (pedido => pedido.estado === "PAGADO" && pedido.cliente.ciudad === "Garzón")
.map(pedido => ({
    factura: `FAC-${pedido.id}`,         
    cliente: pedido.cliente.nombre,
    cantidadArticulos: pedido.items.reduce((total, items) => total + items.cant, 0),        
...
conteoPorEstado = pedidos.reduce((conteo, pedido) => {
  conteo[pedido.estado] = (conteo[pedido.estado] ||0) +1;
  return conteo;
}, {});

ticketPromedio = conteoPorEstado.PAGADO === 0 ? 0 : recaudoEfectivo / conteoPorEstado.PAGADO;
...
async function validarCarritoCompleto(itemsCarrito) {
  try {
    ...
    const totalPagar = resultados.reduce((acumulado, item) => acumulado + item.subtotal, 0);
    return { ok: true, item: resultados, total: totalPagar};
  } catch (err) {
