...
// TODO: Escribe tu código aquí:
 ...
 pedidosGarzonDTO = pedidos.filter(pedidos => pedidos.estado === "PAGADO" && pedidos.cliente.ciudad === "Garzón").map((elemento) => {
  return {
...
recaudoEfectivo = productoPagados.reduce((acc, actual) => {
  return acc + actual.total
}, 0);

conteoPorEstado = pedidos.reduce((acc, acutual) => {
  if(!acc[acutual.estado]){
    acc[acutual.estado] = 1
  } else{
    acc[acutual.estado] += 1
  }
  return acc
...
async function procesarVentaExpress(sku, cantidad) {
  ...
  try{
    ...
    console.log(`Exito la cantidad es: ${resultado.cantidad} y el subtotal es: ${resultado.subtotal}`)
