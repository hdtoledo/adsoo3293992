...
ORDER BY precio DESC;   --el parentesis va dentro de la condicion en este caso es entre A y B  
...
user.nombre AS nombre_cliente,
prod.nombre AS producto_nombre,
detalle.cantidad, detalle.precio_unitario, detalle.subtotal 
from pedidos ped 
inner join usuarios user      on ped.usuario_id = user.id
inner join detalle_pedidos detalle on ped.id = detalle.pedido_id
inner join productos prod   on detalle.producto_id = prod.id
...
-- resolver lo faltante despues del reto 5 
--pregunta en desencso limite 1 el cliente q mas comrp 