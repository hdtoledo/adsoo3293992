...
let animalesZoo = [
    { id: 1, especie: "Loxodonta africana", nombre: "Elefante", genero: "Macho", edad: 18, tamaño: 'grande' },
    { id: 2, especie: "Didelphis marsupialis", nombre: "Zarigueya", genero: "Hembra", edad: 3, tamaño: 'pequeño' },
    { id: 3, especie: "Aptenodytes forsteri", nombre: "Pinguino", genero: "Macho", edad: 10, tamaño: 'mediano' },
    { id: 4, especie: "Panthera leo", nombre: "Leon", genero: "Macho", edad: 12, tamaño: 'mediano' },
    { id: 5, especie: "Tursiops truncatus", nombre: "Delfin", genero: "Hembra", edad: 6, tamaño: 'pequeño' }
];
...
app.get('/api/zoologico', (req, res) => {

    const animalesInf = animalesZoo.map(an => {
        return { nombre: an.nombre, genero: an.genero, edad: an.edad }
    })

    res.status(200).json({
        ok: true,
        cantidadAnimales: animalesZoo.length,
        animales: animalesInf
    })
})
...
app.get('/api/animal', (req, res) => {
    ...
    if (!nombre) {
        return res.status(400).json({
            ok: false,
            error: 'No se llenaron correctamente los campos // nombre //'
        })
    }

    const animalesFilter = animalesZoo.filter(an => an.nombre.toLocaleLowerCase() === nombre.trim().toLocaleLowerCase())

    if (!animalesFilter.length) {
        return res.status(404).json({
            ok: false,
            error: `No se encontro ningun animal con el nombre: ${nombre}...`
        })
    }
    ...
    res.status(200).json({
        ok: true,
        totalAnimales: animalesFilter.length,
        animales: animalesFilter
    })
})
...
app.get('/api/animal/:id', (req, res) => {
    ...
    if (!id) {
        return res.status(400).json({
            ok: false,
            error: 'No se llenaron correctamente los campos // id //'
        })
    }

    const animalFound = animalesZoo.find(an => an.id === id)

    if (!animalFound) {
        return res.status(404).json({
            ok: false,
            error: `no existe el animal con el id: ${id}`
        })
    }
...
app.post('/api/animal', (req, res) => {
    ...
    const { especie, nombre, genero, edad, tamano } = req.body

    const generosPosibles = ['macho', 'hembra']

    if (!nombre || !genero || !edad || !tamano) {
        return res.status(400).json({
            ok: false,
            error: 'Se deben de llenar todos los campos requeridos correctamente...'
        })
    }
    ...
    if (typeof tamano !== 'string') {
        return res.status(400).json({
            ok: false,
            error: 'El tamaño no debe ser algo distinto a text...'
        })
    }

    if (!generosPosibles.includes(genero.toLocaleLowerCase())) {
        return res.status(400).json({
            ok: false,
            error: 'Es unicamente valido genero macho y hembra...'
        })
    }

    const newAnimal = {
        id: animalesZoo.length ? animalesZoo.length + 1 : 1,
        especie: especie || `¡Nuevo!: ${nombre}ditus`,
        nombre: nombre,
    ...
    animalesZoo.push(newAnimal)

    res.status(201).json({
        ok: true,
        mensaje: 'Se añadio el animal correctamente',
        animalesZoo: animalesZoo.length,
        animalAdd: newAnimal
    })
})
...
app.patch('/api/animal/:id/edad', (req, res) => {
    ...
    if (id <= 0 || Number(edad) <= 0) {
        return res.status(400).json({
            ok: false,
            error: 'El id y la edad no puede ser menor o igual a 0...'
        })
    }
    ...
    const anterior = animalFound.edad

    animalFound.edad = edad

    res.status(200).json({
        ok: true,
        edadAnterior: anterior,
        modificado: animalFound
    })
...
app.delete('/api/animal/:id', (req, res) => {
    ...
    const index = animalesZoo.findIndex(an => an.id === id)
    ...
    const [eliminado] = animalesZoo.splice(index, 1)

    res.status(200).json({
        ok: true,
        animalEliminado: eliminado,
        animalesTotales: animalesZoo.length
    })
...
app.use((req, res) => {
    res.status(404).json({
        ...
        metodo: req.method,
        sugerencia: 'apagar y prender el computador'
    })
})
