...
pedidosGarzonDTO = pedidos.filter(pedido => pedido.estado === 'PAGADO' && pedido.cliente.ciudad === 'Garzón').map(pedido => {
...
ticketPromedio = sumaTotal / pedidosPagados.length
...
function verificarStockBodega(sku, cantidadDeseada) {
    return new Promise((resolve, reject) => {
        ...
        setTimeout(() => {
            ...
            const item = inventario.find(prd => prd.sku === sku) 
            ...
            if(item && item.stock < cantidadDeseada){
                return reject(`[409_OUT_OF_STOCK] Stock insuficiente para '${item.nombre}'. Disponible: ${item.stock}, Solicitado: ${cantidadDeseada}`)
            }

            return resolve({sku: item.sku, nombre: item.nombre, categoria: item.categoria, cantidad: cantidadDeseada, precioUnitario: formatoCOP(item.precio), subtotal: formatoCOP(item.precio * cantidadDeseada), disponible: true})
...
async function procesarVentaExpress(sku, cantidad) {
    try{
        ...
        console.log(`✅La cantidad total es: ${resultado.cantidad} && El subtotal es: ${resultado.subtotal}`);
        
        return { exito: true, datos: resultado }

    } catch(error) {
        console.error("❌ Fallo en venta:", error)
        return { exito: false, motivo: error }
