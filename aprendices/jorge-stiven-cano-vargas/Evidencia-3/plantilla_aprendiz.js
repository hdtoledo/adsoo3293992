...
.map(p => {
   const cantidadArticulos = p.items.reduce((acc, item) => acc + item.cant, 0);
   return {
   factura: `FAC-${p.id}`,
...
recaudoEfectivo = pedidos
.filter(p => p.estado === "PAGADO")
.reduce((acc, p) => acc + p.total, 0);

conteoPorEstado = pedidos.reduce((acc, p) => {
acc[p.estado] = (acc[p.estado] || 0) + 1;
return acc;
}, {});

const totalPedidosPagados = pedidos.filter(p => p.estado === "PAGADO").length;
ticketPromedio = totalPedidosPagados > 0 ? recaudoEfectivo / totalPedidosPagados : 0;
...
try {
   ...
   console.log(`✅ Venta exitosa de ${resultado.nombre}. Subtotal: ${formatoCOP(resultado.subtotal)}`);
