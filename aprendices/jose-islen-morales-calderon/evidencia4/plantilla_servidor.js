...
// -----------------------------------------------------------------------------
let productos = [
  { id: 1, nombre: "Platanos", categoria: "VERDURA", stock: 30, precio: 1200 },
  { id: 2, nombre: "Manzana", categoria: "FRUTA", stock: 20, precio: 3100 },
  { id: 3, nombre: "Naranja", categoria: "FRUTA", stock: 15, precio: 2500 },
  { id: 4, nombre: "Cebolla", categoria: "VERDURA", stock: 40, precio: 2200 },
  { id: 5, nombre: "Mango", categoria: "FRUTA", stock: 15, precio: 3500 }
];
...
app.get('/api/productos', (req, res) => {
  ...
  if (categoria) {
    resultado = productos.filter(p => p.categoria.toUpperCase() === categoria.toUpperCase())
  }
  res.status(200).json({
    ok: true,
...
app.post('/api/productos', (req, res) => {
  ...
  if (!nombre || !categoria || !precio || !stock || precio <= 0 || stock < 0) {
    return res.status(400).json({
      ok: false,
      error: "El nombre, categoría, stock y precio son obligatorios."
    })
  }
  const nuevo = {
...
app.patch('/api/productos/:id/stock', (req, res) => {
  ...
  if (typeof nuevoStock !== 'number') {
    return res.status(404).json({
      ok: false,
      error: "Nuevo stock debe ser mayor o igual <= a 0"
    })
  }
