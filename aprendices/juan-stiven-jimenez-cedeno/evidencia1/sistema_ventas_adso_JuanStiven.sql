...
SELECT nombre, email, ciudad, activo, rol_id
from usuarios
where activo = 1
and ( ciudad = 'garzon' or ciudad = 'neiva')
...
SELECT 
p.codigo as codigo_pedidos,
usr.nombre as nombre_cliente,
pr.nombre as nombre_productos,
dp.cantidad,
dp.precio_unitario,
dp.subtotal
from pedidos p
inner join usuarios usr on p.usuario_id = usr.id
...
-- Enunciado 5A: Encontrar los clientes que se registraron pero NUNCA han realizado un pedido.
  SELECT 
    ...
    usr.ciudad
      from usuarios usr 
      left join pedidos ped on usr.id = ped.usuarios_id 
      where ped.id is null and usr.rol_id = 3;
