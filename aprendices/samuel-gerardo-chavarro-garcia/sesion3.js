...
console.log(
  "Tecnólogo en Análisis y Desarrollo de Software (ADSO) - Ficha 2026",
);
console.log(
  "=================================================================\n",
);
...
const pedidos = [
  ...
  },
  {
    ...
    total: 580000,
    estado: "PENDIENTE",
    items: [{ sku: "MON-02", cant: 1, precioUnit: 580000 }],
  },
  {
    id: 103,
  ...
  },
  {
    ...
    total: 320000,
    estado: "CANCELADO",
    items: [{ sku: "SSD-04", cant: 1, precioUnit: 320000 }],
  },
  {
    id: 106,
    ...
    total: 75000,
    estado: "PAGADO",
    items: [{ sku: "MOU-06", cant: 1, precioUnit: 75000 }],
  },
];
...
pedidoConImpuestos = {
  ...
  despachado: false,
  fachaEmicion: new Date().toISOString(),
};
...
if (
  ...
  titular &&
  pedidoConImpuestos &&
  pedidoConImpuestos !== pedidos[0]
) {
  console.log(`✅ [PASSED] Reto 1 Superado:`);
  console.log(
    `   Titular: ${titular} | Sede: ${municipio} | Canal: ${canalVenta}`,
  );
  console.log(
    `   Total Base: ${formatoCOP(totalReto1)} | IVA 19%: ${formatoCOP(pedidoConImpuestos.iva)} | Gran Total: ${formatoCOP(pedidoConImpuestos.granTotal)}`,
  );
} else {
...
pedidosGarzonDTO = pedidos
  .filter(
    (pedido) =>
      pedido.estado === "PAGADO" && pedido.cliente.ciudad === "Garzón",
  )
  .map((pedido) => {
    const cantidadArticulos = pedido.items.reduce(
      (total, item) => total + item.cantidad,
      0,
    );
    return {
      factura: `FAC-${pedido.id}`,
...
// Verificación:
if (
  pedidosGarzonDTO.length === 2 &&
  pedidosGarzonDTO[0].factura &&
  pedidosGarzonDTO[0].totalTexto
) {
  console.log("✅ [PASSED] Reto 2 Superado:");
  console.table(pedidosGarzonDTO);
} else {
  console.log(
    "❌ [FAILED] Reto 2 Pendiente: Debe retornar 2 pedidos pagados de Garzón estructurados como DTOs.",
  );
}
console.log("");
...
recaudoEfectivo = pedidos.reduce((acumulado, pedido) => {
  return pedido.estado === "PAGADO" ? acumulado + pedido.total : acumulado;
}, 0);
// conteoPorEstado = pedidos.reduce(...);
conteoPorEstado = pedidos.reduce((acumulado, pedido) => {
  acumulado[pedido.estado] = (acumulado[pedido.estado] || 0) + 1;
  return acumulado;
}, {});
// ticketPromedio = ...;
...
// Verificación:
if (
  recaudoEfectivo === 1583000 &&
  conteoPorEstado.PAGADO === 4 &&
  conteoPorEstado.CANCELADO === 1
) {
  ...
  console.log(
    "❌ [FAILED] Reto 3 Pendiente: Revisa el recaudo (esperado: $1.583.000 COP) y conteo por estados.",
  );
}
console.log("");
  ...
  return new Promise((resolve, reject) => {
    ...
    setTimeout(() => {
      ...
      if (!item) {
        return reject(
          new Error(
            `[404_NOT_FOUND] El producto con SKU '${sku}' no existe en bodega.`,
          ),
        );
      }
      ...
      if (item.stock < cantidadDeseada) {
        return reject(
          new Error(
            `[409_OUT_OF_STOCK] Stock insuficiente para '${item.nombre}'. Disponible: ${item.stock}, Solicitado: ${cantidadDeseada}`,
          ),
        );
      }
...
console.log(
  "👉 Para probar tu función del Reto 4, se ejecutará dentro del Reto 5 y 6.\n",
);
...
async function procesarVentaExpress(sku, cantidad) {
  ...
  try{
    ...
    console.log(`venta exitosa: ${resultado.nombre} * ${resultado.cantidad} => total: ${formatoCOP(resultado.subtotal)}` 
  );
...
console.log(
  "📌 RETO 6 (Nivel PRO): Verificación de Carrito en Paralelo con Promise.all",
);
console.log(
  "-----------------------------------------------------------------",
);
...
async function ejecutarSuiteDePruebas() {
  ...
  if (t6Valido.ok) {
    console.log(
      `✅ [PASSED] Carrito verificado en paralelo. Total a pagar: ${formatoCOP(t6Valido.totalPagar)}`,
    );
  } else {
  ...
  if (!t6Falla.ok) {
    console.log(
      `✅ [PASSED] Fail-Fast correcto: Se abortó la compra por: ${t6Falla.error}`,
    );
  } else {
