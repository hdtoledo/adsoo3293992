 ...
 * APRENDIZ: JUAN CARLOS GUERRORE MENDEZ
 * =============================================================================
...
const {
  id: nroPedid,
  cliente: {nombre: titula, ciudad: municipi},
  total: totali,
  canalVenta: canalVent = "PORTAL_WEB"
} = pedidos[0];
...
// TODO: Escribe tu código aquí:
const recaudoEfectivo = pedidos.reduce((acc, pedidos) => {return pedidos.estado === "PAGADO" ? acc + pedidos.total : acc}, 0);


const conteoPorEstado = pedidos.reduce((acc, pedidos) => {
  const est = pedidos.estado;
  acc[est] = (acc[est] || 0) + 1; 
  return acc;
}, {});


const ticketPromedio = recaudoEfectivo / conteoPorEstado.PAGADO;
...
console.log(verificarStockBodega)
