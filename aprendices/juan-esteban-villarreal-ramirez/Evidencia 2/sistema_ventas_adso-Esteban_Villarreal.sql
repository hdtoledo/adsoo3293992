...
-- Modificación 1: Cambia un INNER JOIN por un LEFT JOIN en tu pantalla y explica por qué aparecieron valores NULL.
-- R/ porque el inner join solo trae datos completos que en ningun lado sea nulos a diferencia de
-- LEFT JOIN ya que solo lee datos desde la izquierda
...
-- Modificación 2: Mueve una condición de HAVING a WHERE y explica al instructor por qué el motor arroja error de sintaxis.
-- R/ Sale error syntax porque el where filtra antes de agrupar los datos y el having despues de agrupar
...
FROM pedidos
GROUP BY estado
WHERE SUM(total) > 1500000.00;

-- Modficación 3: Obtén el cliente con mayor volumen histórico de compras usando ORDER BY DESC LIMIT 1.
-- R/ El cliente con la mayor venta es Carlos Andres Perdomo (deberia ganarse la rifa)

SELECT 
	u.id,
    u.nombre,
SUM(total) AS mayor_venta
FROM usuarios u
INNER JOIN pedidos p ON u.id = p.usuario_id
GROUP BY u.id, u.nombre
ORDER BY mayor_venta DESC LIMIT 1;