...
// -----------------------------------------------------------------------------
let productos = [
    { id: 1, nombre: "tacometros", tipoderepuesto: "original", stock: 8, precio: 150000},
    { id: 2, nombre: "direccionales", tipoderepuesto: "original", stock: 3, precio: 80000 },
    { id: 3, nombre: "manubrios", tipoderepuesto: "original", stock: 3, precio: 25000 },
    { id: 4, nombre: "llantas", tipoderepuesto: "generico", stock: 5, precio: 320000 },
    { id: 5, nombre: "kit de arrastre", tipoderepuesto: "generico", stock: 0, precio: 80000 }
];
...
app.get('/api/status', (req, res) => {
    res.status(200).json({
        ok: true,
        message: "serverfuncionando correctmente",
...
// });
    ...
    const { tipoderepuesto } = req.query;
    let resultado = productos;


    if (tipoderepuesto) {
        resultado = productos.filter(p =>
            p.tipoderepuesto.toUpperCase() === tipoderepuesto.toUpperCase()
        );
    }
    ...
    res.status(200).json({
        ...
        filtros: { tipoderepuesto: tipoderepuesto || "todos" },
        datos: resultado
    });
});
...
app.post('/api/productos', (req, res) => {
    ...
    const { nombre, tipoderepuesto, stock, precio } = req.body

    console.log(req.body);
    

    if (!nombre || !tipoderepuesto || !stock || precio <=0 ){
    ...
    const nuevo  = {
        ...
        nombre: nombre.trim(),
        tipoderepuesto: (tipoderepuesto || "GENERAL").toUpperCase(),
...
// app.patch('/api/productos/:id/stock', (req, res) => { ... });
    ...
    if (typeof nuevostock !== 'number'){
        return res.status(404).json({
            ok: false,
            error: "El nuevo stock debe ser mayor o igual a 0"
        });
    }
...
// app.delete('/api/productos/:id', (req, res) => { ... });
    ...
    if (index === -1) {
        return res.status(404).json({
            ok: false,
            error: "Producto con ID no encontrado para eliminar"
        });
    }
...
// app.use((req, res) => { ... });
    ...
    res.status(404).json({
    ok: false,
    error: "Endpoint no encontrado en la taller",
    metodo: req.method,
